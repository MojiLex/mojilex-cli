...
настроек. Если конфигурация уже существует, повторно запускать `init` не нужно:
...
Если `ready` имеет значение `false`, установите компонент, указанный в отчёте:
...
В интерактивном терминале команды `add`, `import`, `describe` и `resume` скрыто
запрашивают отсутствующие учетные данные непосредственно перед их использованием.
Значения существуют только внутри текущего процесса и не сохраняются на диск.
...
Для CI, `--json`, `--quiet` и `--non-interactive` задайте учетные данные через
...
На Windows поле `install_commands` содержит одну готовую PowerShell-команду для
установки отсутствующих медиакомпонентов. Она может запросить права
администратора и установить Visual Studio Build Tools с C++ workload.
