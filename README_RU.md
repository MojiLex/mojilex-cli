...
Внутри каждого этапа паки обрабатываются последовательно. Временные ошибки медиа
повторяются автоматически (по умолчанию до шести попыток). Если ошибка остаётся,
очередь останавливается перед следующим паком; сохранённую работу можно продолжить.
Скачивания, декодирование и запросы ИИ внутри текущего пака остаются параллельными.
Старый параметр `processing.pack_concurrency` по-прежнему читается, проверяется
и доступен для изменения ради совместимости с существующими конфигурациями и
автоматизацией, но его изменение не запускает несколько паков одновременно и
не нарушает последовательность очереди.

**Параллельные скачивания**, **Параллельные декодеры медиа** и **Параллельные
запросы к ИИ** управляют одновременной работой внутри текущего пака. Сохранённые
лимиты запросов и стоимости ИИ, а также временного хранилища медиа действуют на
всю операцию со всеми последовательно поставленными в очередь паками. Обновления
...
| Как ускорить? | Паки проходят очередь последовательно. Внутри текущего пака можно увеличить лимиты скачиваний, декодеров и ИИ. Эти лимиты настраиваются отдельно под компьютер и провайдера. Для существующего запуска `mojilex resume NewsEmoji --ai-concurrency 4` меняет параллельность ИИ без увеличения бюджета запросов. |
