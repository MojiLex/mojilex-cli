...
Во время `add`, `import`, `describe` и `resume` CLI показывает счётчик готовых
файлов, процент, ошибки, время и активный этап (скачивание, обработка или ожидание
AI). Если работа долгая, строка обновляется каждые 5 секунд; «без завершений»
...
То же правило действует для AI: при ошибке или отказе от расходов очередь
останавливается, а результаты уже начавшихся успешных пачек сохраняются.
...
Параметр доступен и для `add`. Без него сохранённый запуск использует прежнюю
...
Завершённые файлы импорта сохраняются по одному: при продолжении их содержимое
повторно проверяется, но пригодный сохранённый анализ не вычисляется заново.
