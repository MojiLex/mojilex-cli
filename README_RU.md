...
продолжаются. Для явной проверки обновлений источника используйте `mojilex import URL --refresh`.
...
зависимости, освобождая слоты подготовки для остальных. Результаты объединяются
в порядке входного списка внутри каждого сохранённого запуска.

| Режим | Порядок |
|---|---|
| `fast` — по умолчанию | Совмещать скачивание, обработку на ПК и ИИ разных паков. |
...
несовместимый результат запрашивается заново. Литералы с `<` или `>` (например,
`</>`) текущая публичная схема не допускает: программа не подменяет такие символы.
