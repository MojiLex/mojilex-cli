...
После настройки управляйте сохранёнными паками по имени:
...
`list` показывает паки и прогресс. `show` выводит готовые описания, движение,
примеры использования, теги и предупреждения прямо в терминале. `review` —
добровольный просмотр тех же данных, без изменения статусов и без обязательного
одобрения. Просмотр работает локально и не отправляет запросы модели.
