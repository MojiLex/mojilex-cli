...
class ModelBenchmarkManifest(BaseModel):
    ...
    def manifest_integrity(self) -> ModelBenchmarkManifest:
        ...
        if (
            any(
                item.adjudication is not None and item.adjudication.reviewer_count == 1
...
async def run_model_benchmark(
    ...
    *,
    ...
    for case in manifest.cases:
        ...
        request = _request_for_case(manifest, case, image_bytes)
...
def evaluate_model_benchmark(
    ...
    *,
    ...
    if set(by_id) != expected_ids or len(by_id) != len(observations):
        raise BenchmarkError("model observations must cover every case exactly once")
    evaluated = tuple(
        _CaseEvaluation(
            case=case,
            ...
            adjudication=(
                ...
                and case.adjudication.response_sha256 == by_id[case.case_id].response_sha256
                else None
            ),
        )
        for case in manifest.cases
    ...
    counts = {
        name: sum(name in item.case.strata for item in evaluated) for name in MANDATORY_MODEL_STRATA
    }
    aggregate_strata = {
    ...
    gates = {
        "benchmark_has_at_least_240_items": len(evaluated) >= 240,
        ...
        "complex_holdout_has_at_least_3_runs": manifest.holdout_run_count >= 3,
    ...
    return finalize_report(
        {
            ...
            "holdout_run_count": manifest.holdout_run_count,
...
def _metrics(evaluated: tuple[_CaseEvaluation, ...]) -> dict[str, Any]:
    ...
    hallucination_cases = {
        item.case.case_id
        for item in evaluated
        if item.adjudication is None or item.adjudication.hallucinated_observable_fact
    }
    brand_false = sum(
    ...
    for item in evaluated:
        ...
        if response is not None and any(
            claim.casefold() in output_text for claim in item.case.forbidden_claims
        ):
            hallucination_cases.add(item.case.case_id)
    ...
    return {
        ...
        "hallucination_bp": ratio_bp(len(hallucination_cases), count),
...
def _macro_f1_bp(pairs: Sequence[tuple[set[str], set[str], int]]) -> int:
    if not pairs:
        return 0
    weighted = sum(
        _f1_bp(len(expected & actual), len(actual - expected), len(expected - actual)) * weight
        for expected, actual, weight in pairs
    )
    return ratio_bp(weighted, sum(weight for _, _, weight in pairs) * 10_000)
